...
import net.musketeer.datasync.protocol.jms.config.JmsProtocolConfig;
 ...
 * Date Nov 5, 2013
 */
public class JmsConfigProcessor implements ProtocolConfigProcessor {
	...
	static {
		...
		sb.append( ":" ).append( JmsProtocolConfig.class.getPackage().getName() );
	...
	public ProtocolConfig read( InputStream is ) throws Exception {
		...
		final ProtocolConfig config = ( JmsProtocolConfig ) unmarshaller.unmarshal( is );
		return config;
	}
